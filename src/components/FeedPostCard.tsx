  ...
  return (
    <Card>
      <div className="flex items-start justify-between gap-4">
        ...
        <button className="rounded-full p-2 text-slate-500 hover:bg-white/5 hover:text-white" aria-label="More actions">
      ...
      <div className="mt-5 flex items-center gap-4 border-t border-slate-800 pt-4 text-sm text-slate-400">
        <button className="flex items-center gap-2 hover:text-white"><Heart className="h-4 w-4" />{formatCompact(post.likes)}</button>
        <button className="flex items-center gap-2 hover:text-white"><MessageCircle className="h-4 w-4" />{formatCompact(post.comments)}</button>
        <button className="flex items-center gap-2 hover:text-white"><Bookmark className="h-4 w-4" />{formatCompact(post.bookmarks)}</button>
      </div>
    </Card>
  );
}
