  ...
  return (
    <Shell active={active}>
      <div className="space-y-6 pb-24">
        {/* Use a simple header without tags to reduce visual clutter */}
        <section className="rounded-[2.5rem] border border-blue-400/20 bg-blue-500/10 p-8">
          {/* Display eyebrow text as subtle overline text instead of a badge */}
          {eyebrow ? <p className="text-xs uppercase tracking-wide text-blue-300">{eyebrow}</p> : null}
          <h1 className="mt-2 max-w-4xl text-4xl font-black tracking-tight text-white md:text-5xl">{title}</h1>
        ...
        {cards.length ? (
          <section>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {cards.map((card) => (
                <Card key={card.title}>
                  {/* Omit badge display for guideline cards to reduce visual clutter */}
