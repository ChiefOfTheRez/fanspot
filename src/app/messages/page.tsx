...
export default function MessagesPage() {
  ...
  useEffect(() => {
    try {
      ...
      const storedConvs = localStorage.getItem(`fanspot-conversations-${userKeyPrefix}`);
      const storedMessages = localStorage.getItem(`fanspot-messages-${userKeyPrefix}`);
      if (storedConvs) {
  ...
  useEffect(() => {
    try {
      const userKeyPrefix = session?.user?.email ? `${session.user.email}-` : "";
      localStorage.setItem(`fanspot-conversations-${userKeyPrefix}`, JSON.stringify(conversations));
      localStorage.setItem(`fanspot-messages-${userKeyPrefix}`, JSON.stringify(messagesByConversation));
    } catch (err) {
      // no-op
    }
  }, [conversations, messagesByConversation, session]);
